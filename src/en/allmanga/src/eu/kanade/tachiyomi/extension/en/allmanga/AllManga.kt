package eu.kanade.tachiyomi.extension.en.allmanga
...
/**
 * Primary Manga Source: AllManga.to (mkissa.to)
 *
 * Sourced directly from keiyoushi/extensions-source (src/en/allanime module)
 * Retains exact upstream GraphQL queries, pagination models, and CDN handling,
 * packaged for ShonenX's dartotsu_extension_bridge and Mihon.
 */
@Source
abstract class AllManga :
    ...
    override suspend fun getMangaUpdate(manga: SManga, chapters: List<SChapter>): SMangaUpdate {
        var retries = 0
        while (true) {
            try {
                val payload = graphQLBody(
                ...
                val data = client.post(apiUrl, payload).parseGraphQLAs<MangaUpdateData>()
                val mangaData = data.manga?.toSManga()
                    ?: throw Exception("Manga data missing")

                val chapterList = data.chapterList.map {
                    it.toSChapter(
                        mangaId = manga.url,
                        mangaSlug = manga.memo["slug"]?.string,
                    )
                }
                ...
                if (retries++ < MAX_RETRIES) {
                    val retryAfter = retryAfterRegex.find(e.message)?.groupValues?.get(1)?.toLongOrNull()
                    delay(retryAfter?.times(1000) ?: (RETRY_DELAY_MS * retries))
                    continue
                }
                throw e
            }
    ...
    override suspend fun getRelatedMangaList(manga: SManga): List<SManga> {
        val related = manga.memo["relatedMangas"]
            ?.parseAs<List<Related>>()
            ?: return emptyList()
        ...
        val fewerGenres = genres.take(2)

        fun searchPayload(genres: List<String>) = SearchPayload(
            query = null,
            sortBy = null,
            genres = genres,
            excludeGenres = null,
    ...
    override fun getChapterUrl(chapter: SChapter): String {
        if (chapter.url.startsWith("/")) {
            // Legacy URL structure: /read/$mangaId/$mangaSlug/chapter-$chapterNum-sub
